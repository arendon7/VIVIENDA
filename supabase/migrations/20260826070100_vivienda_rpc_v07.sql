-- VIVIENDA v0.7 — narrow RPC surface over private persistence tables.
-- All persistence RPCs are service-role only. End-user JWTs may only resolve their opaque principal.
...
declare
  ...
  v_evidence_refs text[] := array(
    select jsonb_array_elements_text(coalesce(v_event->'evidenceRefs', '[]'::jsonb))
  );
begin
...
-- Server-side identity provisioning; no PII is stored here.
...
as $$
begin
  insert into private.vivienda_identity_subjects(auth_user_id, subject_ref, principal_kind, status)
  values (p_auth_user_id, p_subject_ref, p_principal_kind, 'active')
...
-- The only v0.7 function intended for a normal authenticated JWT.
...
begin
  ...
  if found then
    ...
    return jsonb_build_object('kind','duplicate','snapshot',private.vivienda_case_snapshot(v_existing.case_id));
  end if;
  ...
  return jsonb_build_object('kind','created','snapshot',private.vivienda_case_snapshot(p_case_id));
exception
...
begin
  ...
  if not exists (
    select 1
    from private.vivienda_identity_subjects
    where subject_ref = p_lawyer_subject_ref
      and principal_kind = 'lawyer'
      and status = 'active'
  ) then
...
begin
  ...
  where authorization_id = (
    ...
    where a.case_id = p_case_id
      and a.subject_ref = p_subject_ref
      and a.status = 'active'
...
begin
  if not exists (
    select 1 from private.vivienda_cases where case_id = p_intent->>'caseId'
  ) then
    raise exception 'vivienda:case_not_found' using errcode = 'P0001';
  end if;
  ...
  if not exists (
    ...
    where a.case_id = p_intent->>'caseId'
      ...
      and a.status = 'active'
      and a.purposes && array[
        'mortgage_analysis','case_management','legal_service','external_credit_data'
      ]::text[]
  ) then
  ...
  insert into private.vivienda_evidence_intents(
    ...
    p_intent->>'evidenceId',
    p_intent->>'caseId',
    p_intent->>'createdBySubjectRef',
    p_intent->>'kind',
    p_intent->>'legalDataCategory',
    p_intent->>'securityTier',
    p_intent->>'displayName',
    (p_intent->>'createdAt')::timestamptz,
    (p_intent->>'expiresAt')::timestamptz,
    p_intent->>'status'
  );
end;
$$;
...
-- Physical coordinates are registered separately from the CasePersistencePort.
...
begin
  ...
  from private.vivienda_evidence_intents
  where intent_id = p_intent_id
  for update;

  if not found then
    raise exception 'vivienda:evidence_intent_not_found' using errcode = 'P0001';
  end if;
  ...
  insert into private.vivienda_evidence_objects(
    storage_locator, intent_id, evidence_id, bucket_id, object_path
  ) values (
    p_storage_locator, p_intent_id, v_intent.evidence_id, 'vivienda-evidence', p_object_path
  );
end;
$$;
...
begin
  ...
  select * into v_intent
  from private.vivienda_evidence_intents
  where intent_id = p_intent_id and case_id = p_case_id
  for update;
  ...
  insert into private.vivienda_evidence_metadata(
    ...
    p_metadata->>'evidenceId',
    p_case_id,
    p_metadata->>'kind',
    p_metadata->>'legalDataCategory',
    p_metadata->>'securityTier',
    p_metadata->>'displayName',
    p_metadata->>'mimeType',
    (p_metadata->>'byteSize')::bigint,
    p_metadata->>'checksumSha256',
    v_locator,
    (p_metadata->>'createdAt')::timestamptz,
    p_metadata->>'createdBySubjectRef',
    p_metadata->>'lifecycle',
    null,
    null
  );
...
-- Returns physical coordinates for a storage cleanup worker; it does not mutate Case Log.
...
begin
  ...
  left join private.vivienda_evidence_objects o
    on o.intent_id = e.intent_id and o.deleted_at is null;
end;
$$;

-- Default function EXECUTE privileges are too broad for this data plane.
